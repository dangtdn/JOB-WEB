				...
				<form onSubmit={handleSubmit(addExperiences)}>
					<div
						className={`mt-3 w-full border-2 ${
							errors && errors.type ? 'border-red-400' : 'border-themeLighter'
						...
						{experiences.length > 0 && (
							<>
								{experiences.map((tag: any, index: any) => (
									<span
										key={index}
										className="py-1 pl-3 pr-2 flex justify-between items-center gap-2 bg-green-100 text-themeLight">
...
const PreviewMode = ({ data }: { data: any }) => {
	...
	return (
		<PreviewWrapper className="rounded-lg bg-green-100">
			...
			<PreviewBody className="p-4 overflow-y-auto overflow-x-hidden">
		...
		<div className="relative">
			<form onSubmit={searchHandler}>
				...
				<input
					...
					className="border-2 border-green-100 bg-green-100 text-themeLight rounded-lg focus:outline-none w-full pl-9 pr-3 py-2"
					placeholder="Search"
				/>
			</form>
		</div>
